...
class ProteinInference:
    # Class-level cache for ESM model
    _esm_model = None
    _alphabet = None
    ...
    def __init__(self, sequence):
        ...
        if ProteinInference._esm_model is None:
            logger.info("Loading ESM-2 model esm2_t33_650M_UR50D (this happens once)...")
            ProteinInference._esm_model, ProteinInference._alphabet = esm.pretrained.esm2_t33_650M_UR50D()
            ProteinInference._esm_model.eval()
            ProteinInference._batch_converter = ProteinInference._alphabet.get_batch_converter()
            logger.info("ESM-2 model successfully loaded!")
            
        self.esm_model = ProteinInference._esm_model
        self.alphabet = ProteinInference._alphabet
        self.batch_converter = ProteinInference._batch_converter
